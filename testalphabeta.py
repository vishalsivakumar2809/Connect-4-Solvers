...
def alpha_beta(board, player, alpha, beta, ply):
	...
	for move in board.available_moves():
		...
		# updating our board, then recursively calling alpha_beta to retrieve a score, 
		...
		score, result_move, expanded = alpha_beta(board, switch_player(player), alpha, beta, ply - 1)
		board.undo_move(move)

		expansions = expansions + expanded

		# if player is O, we must minimize the score, so we check if score's lesser than
		# bestScore, to which we update bestScore and bestMove. As it's alpha-beta pruning,
		# we must check if alpha is greater than bestScore, and prune if it is or update
		# beta accordingly.
		...
		# bestScore, to which we update bestScore and bestMove. As it's alpha-beta pruning,
		# we must check if beta is lesser than bestScore, and prune if it is or update
		# alpha accordingly.
...
class TestMinMaxDepth5(unittest.TestCase):
	...
	def test_depth5g(self):
		...
		self.assertEqual(bestScore, 1)
		self.assertEqual(bestMove, 3)		
		print('\nAlpha Beta Expansions: ',expansions)
